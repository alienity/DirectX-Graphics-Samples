...
void VolumeBuffer::CreateDerivedViews(ID3D12Device* Device, DXGI_FORMAT Format, uint32_t DepthSize, uint32_t NumMips)
{
    ...
    UAVDesc.Texture3D.WSize = (UINT)DepthSize;
    ...
    // Create the UAVs for each mip level (RWTexture3D)
...
D3D12_RESOURCE_DESC VolumeBuffer::DescribeTex3D(uint32_t Width, uint32_t Height, uint32_t DepthOrArraySize,
...
void VolumeBuffer::Create(const std::wstring& Name, uint32_t Width, uint32_t Height, uint32_t Depth, uint32_t NumMips,
...
{
    Create(Name, Width, Height, Depth, NumMips, Format);
}

void VolumeBuffer::GenerateMipMaps(CommandContext& BaseContext)
{
    if (m_NumMipMaps == 0)
        return;
    ...
    for (uint32_t TopMip = 0; TopMip < m_NumMipMaps; )
    {
        ...
        uint32_t SrcDepth = m_ArraySize >> TopMip;
