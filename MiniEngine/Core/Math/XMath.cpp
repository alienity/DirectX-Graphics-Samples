...
namespace Math
{
    ...
    {
        // Heron's formula:
        XMVECTOR a = XMVector3Length(XMVectorSubtract(B, A));
        XMVECTOR b = XMVector3Length(XMVectorSubtract(C, A));
        XMVECTOR c = XMVector3Length(XMVectorSubtract(C, B));
        XMVECTOR p = XMVectorMultiply(XMVectorAdd(XMVectorAdd(a, b), c), XMVectorReplicate(0.5f));
        XMVECTOR areaSq = XMVectorMultiply(XMVectorMultiply(XMVectorMultiply(p, XMVectorSubtract(p, a)), XMVectorSubtract(p, b)), XMVectorSubtract(p, c));
        float area;
    ...
    )
    {
        float x, y, z, t;
        t = atInterval;
        x = (2 * t * t * t - 3 * t * t + 1) * startPos.x + (-2 * t * t * t + 3 * t * t) * endPos.x + (t * t * t - 2
            * t * t + t) * startTangent.x + (t * t * t - t * t) * endTangent.x;
        y = (2 * t * t * t - 3 * t * t + 1) * startPos.y + (-2 * t * t * t + 3 * t * t) * endPos.y + (t * t * t - 2
            * t * t + 1) * startTangent.y + (t * t * t - t * t) * endTangent.y;
        z = (2 * t * t * t - 3 * t * t + 1) * startPos.z + (-2 * t * t * t + 3 * t * t) * endPos.z + (t * t * t - 2
            * t * t + 1) * startTangent.z + (t * t * t - t * t) * endTangent.z;
    ...
    {
        float roll = atan2f(2 * quaternion.x * quaternion.w - 2 * quaternion.y * quaternion.z,
                            1 - 2 * quaternion.x * quaternion.x - 2 * quaternion.z * quaternion.z);
        float pitch = atan2f(2 * quaternion.y * quaternion.w - 2 * quaternion.x * quaternion.z,
    ...
    float GetPointSegmentDistance(const XMVECTOR& point, const XMVECTOR& segmentA, const XMVECTOR& segmentB)
    {
        ...
        const float l2 = XMVectorGetX(XMVector3LengthSq(XMVectorSubtract(segmentB, segmentA))); // i.e. |w-v|^2 -  avoid a sqrt
        ...
        const float t = std::max(
            0.0f, std::min(1.0f, XMVectorGetX(XMVector3Dot(XMVectorSubtract(point, segmentA), XMVectorSubtract(segmentB, segmentA))) / l2));
        const XMVECTOR projection = XMVectorAdd(segmentA, XMVectorMultiply(XMVectorReplicate(t), XMVectorSubtract(segmentB, segmentA))); // Projection falls on the segment
    ...
    void GetBarycentric(const XMVECTOR& p, const XMVECTOR& a, const XMVECTOR& b, const XMVECTOR& c, float& u,
                        float& v, float& w, bool clamp)
    {
        XMVECTOR v0 = XMVectorSubtract(b, a), v1 = XMVectorSubtract(c, a), v2 = XMVectorSubtract(p, a);
        ...
        if (clamp)
        {
            if (u < 0)
            {
                float t = XMVectorGetX(XMVectorDivide(XMVector3Dot(XMVectorSubtract(p, b), XMVectorSubtract(c, b)), XMVector3Dot(XMVectorSubtract(c, b), XMVectorSubtract(c, b))));
                t = saturate(t);
                u = 0.0f;
                v = 1.0f - t;
                w = t;
            ...
            {
                float t = XMVectorGetX(XMVectorDivide(XMVector3Dot(XMVectorSubtract(p, c), XMVectorSubtract(a, c)), XMVector3Dot(XMVectorSubtract(a, c), XMVectorSubtract(a, c))));
                t = saturate(t);
                u = t;
                v = 0.0f;
                w = 1.0f - t;
            ...
            {
                float t = XMVectorGetX(XMVectorDivide(XMVector3Dot(XMVectorSubtract(p, a), XMVectorSubtract(b, a)), XMVector3Dot(XMVectorSubtract(b, a), XMVectorSubtract(b, a))));
                t = saturate(t);
                u = 1.0f - t;
                v = t;
                w = 0.0f;
