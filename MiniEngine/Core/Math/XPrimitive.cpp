...
namespace Math
{
	namespace Primitive
	{
		...
		{
			XMFLOAT3 minVal = getMin();
			XMFLOAT3 maxVal = getMax();
			return (maxVal.x - minVal.x) * (maxVal.y - minVal.y) * (maxVal.z - minVal.z);
		}
		...
		{
			float distsq = Math::DistanceSquared(XMLoadFloat3(&center), P);
		...
		{
			float distsq = Math::DistanceSquared(center, P);
			float radiussq = radius * radius;
			return distsq < radiussq;
		}
		...
		bool Sphere::intersects(const AABB& b) const
		{
			...
			XMFLOAT3 closestPointInAabb = Math::Min(Math::Max(center, min), max);
			float distanceSquared = Math::DistanceSquared(closestPointInAabb, center);
		...
		{
			dist = Math::Distance(center, b.center);
		...
		bool Sphere::intersects(const Sphere& b, float& dist, XMFLOAT3& direction) const
		{
			...
			XMVECTOR DIR = XMVectorSubtract(A, B);
			XMVECTOR DIST = XMVector3Length(DIR);
			DIR = XMVectorDivide(DIR, DIST);
		...
		bool Sphere::intersects(const Capsule& b, float& dist) const
		{
			...
			XMVECTOR N = XMVector3Normalize(XMVectorSubtract(A, B));
			A = XMVectorSubtract(A, XMVectorMultiply(N, XMVectorReplicate(b.radius)));
			B = XMVectorAdd(B, XMVectorMultiply(N, XMVectorReplicate(b.radius)));
			XMVECTOR C = XMLoadFloat3(&center);
			dist = Math::GetPointSegmentDistance(C, A, B);
			dist = dist - radius - b.radius;
			return dist < 0;
		}
		...
		bool Sphere::intersects(const Capsule& b, float& dist, XMFLOAT3& direction) const
		{
			...
			B = XMVectorAdd(B, XMVectorMultiply(N, XMVectorReplicate(b.radius)));
			XMVECTOR C = XMLoadFloat3(&center);
			XMVECTOR D = XMVectorSubtract(C, Math::ClosestPointOnLineSegment(A, B, C));
			dist = XMVectorGetX(XMVector3Length(D));
			D = XMVectorDivide(D, XMVectorReplicate(dist));
		...
		bool Sphere::intersects(const Ray& b, float& dist, XMFLOAT3& direction) const
		{
			...
			if (discr > 0)
			{
				...
				{
					XMVECTOR P = XMVectorAdd(O, XMVectorMultiply(D, XMVectorReplicate(t)));
		...
		XMFLOAT4X4 Sphere::GetPlacementOrientation(const XMFLOAT3& position, const XMFLOAT3& normal) const
		{
			...
			XMVECTOR E = XMVectorSubtract(XMLoadFloat3(&center), P);
			XMVECTOR T = XMVector3Normalize(XMVector3Cross(N, XMVectorSubtract(P, E)));
		...
		bool Capsule::intersects(const Capsule& other, XMFLOAT3& position, XMFLOAT3& incident_normal,
			float& penetration_depth) const
		{
			...
			XMVECTOR a_Normal = XMVector3Normalize(XMVectorSubtract(a_Tip, a_Base));
			XMVECTOR a_LineEndOffset = XMVectorMultiply(a_Normal, a_Radius);
			XMVECTOR a_A = XMVectorAdd(a_Base, a_LineEndOffset);
			XMVECTOR a_B = XMVectorSubtract(a_Tip, a_LineEndOffset);
			...
			XMVECTOR b_Normal = XMVector3Normalize(XMVectorSubtract(b_Tip, b_Base));
			XMVECTOR b_LineEndOffset = XMVectorMultiply(b_Normal, b_Radius);
			XMVECTOR b_A = XMVectorAdd(b_Base, b_LineEndOffset);
			XMVECTOR b_B = XMVectorSubtract(b_Tip, b_LineEndOffset);

			// Vectors between line endpoints:
			XMVECTOR v0 = XMVectorSubtract(b_A, a_A);
			XMVECTOR v1 = XMVectorSubtract(b_B, a_A);
			XMVECTOR v2 = XMVectorSubtract(b_A, a_B);
			XMVECTOR v3 = XMVectorSubtract(b_B, a_B);
			...
			XMVECTOR bestB = Math::ClosestPointOnLineSegment(b_A, b_B, bestA);

			// Now do the same for capsule A segment:
			bestA = Math::ClosestPointOnLineSegment(a_A, a_B, bestB);

			// Finally, sphere collision:
			XMVECTOR N = XMVectorSubtract(bestA, bestB);
			XMVECTOR len = XMVector3Length(N);
			N = XMVectorDivide(N, len);
			float dist = XMVectorGetX(len);

			XMStoreFloat3(&position, XMVectorSubtract(bestA, XMVectorMultiply(N, XMVectorReplicate(radius))));
		...
		bool Capsule::intersects(const Ray& ray, float& dist, XMFLOAT3& direction) const
		{
			...
			XMVECTOR L = XMVector3Normalize(XMVectorSubtract(A, B));
			A = XMVectorSubtract(A, XMVectorMultiply(L, XMVectorReplicate(radius)));
			B = XMVectorAdd(B, XMVectorMultiply(L, XMVectorReplicate(radius)));
			XMVECTOR O = XMLoadFloat3(&ray.origin);
			XMVECTOR D = XMLoadFloat3(&ray.direction);
			XMVECTOR C = XMVector3Normalize(XMVector3Cross(L, XMVectorSubtract(A, O)));
			XMVECTOR N = XMVector3Cross(L, C);
			XMVECTOR Plane = XMPlaneFromPointNormal(A, N);
			XMVECTOR I = XMPlaneIntersectLine(Plane, O, XMVectorAdd(O, XMVectorMultiply(D, XMVectorReplicate(ray.TMax))));
			XMVECTOR P = Math::ClosestPointOnLineSegment(A, B, I);
		...
		bool Capsule::intersects(const XMFLOAT3& point) const
		{
			...
			XMVECTOR Normal = XMVector3Normalize(XMVectorSubtract(Tip, Base));
			...
			XMVECTOR C = Math::ClosestPointOnLineSegment(A, B, P);

			return XMVectorGetX(XMVector3Length(XMVectorSubtract(P, C))) <= radius;
		}
		...
		XMFLOAT4X4 Capsule::GetPlacementOrientation(const XMFLOAT3& position, const XMFLOAT3& normal) const
		{
			...
			const XMVECTOR Axis = XMVector3Normalize(XMVectorSubtract(Tip, Base));
			XMVECTOR N = XMVector3Normalize(XMLoadFloat3(&normal));
			XMVECTOR P = XMLoadFloat3(&position);
			XMVECTOR E = Axis;
			XMVECTOR T = XMVector3Normalize(XMVector3Cross(N, XMVectorSubtract(P, E)));
		...
		bool Plane::intersects(const Sphere& b, float& dist, XMFLOAT3& direction) const
		{
			...
			dist = Math::GetPlanePointDistance(XMLoadFloat3(&origin), XMLoadFloat3(&normal), C);
			direction = normal;
			if (dist < 0)
			{
				direction.x *= -1;
			...
			if (dist < 0)
			{
				...
				XMVECTOR uvw = XMVectorAdd(XMVectorMultiply(clipSpacePos, XMVectorSet(0.5f, -0.5f, 0.5f, 1)), XMVectorSet(0.5f, 0.5f, 0.5f, 0));
				XMVECTOR uvw_sat = XMVectorSaturate(uvw);
				XMVECTOR uvw_diff = XMVectorAbs(XMVectorSubtract(uvw, uvw_sat));
		...
		bool Plane::intersects(const Capsule& b, float& dist, XMFLOAT3& direction) const
		{
			...
			XMVECTOR A = XMLoadFloat3(&b.base);
			XMVECTOR B = XMLoadFloat3(&b.tip);
			XMVECTOR D = XMVector3Normalize(XMVectorSubtract(A, B));
			A = XMVectorSubtract(A, XMVectorMultiply(D, XMVectorReplicate(b.radius)));
			B = XMVectorAdd(B, XMVectorMultiply(D, XMVectorReplicate(b.radius)));
			...
			else
			{
				...
				XMVECTOR t_vec = XMVector3Dot(N, XMVectorDivide(XMVectorSubtract(A, O), XMVectorAbs(XMVector3Dot(N, D))));
				float t = XMVectorGetX(t_vec);
				XMVECTOR LinePlaneIntersection = XMVectorAdd(A, XMVectorMultiply(D, XMVectorReplicate(t)));
				C = Math::ClosestPointOnLineSegment(A, B, LinePlaneIntersection);
			}

			dist = Math::GetPlanePointDistance(O, N, C);
		...
		bool Plane::intersects(const Ray& b, float& dist, XMFLOAT3& direction) const
		{
			...
			XMVECTOR O = XMLoadFloat3(&b.origin);
			XMVECTOR A = XMVectorAdd(O, XMVectorMultiply(D, XMVectorReplicate(b.TMin)));
			XMVECTOR B = XMVectorAdd(O, XMVectorMultiply(D, XMVectorReplicate(b.TMax)));

			dist = XMVectorGetX(XMVector3Dot(N, XMVectorDivide(XMVectorSubtract(XMLoadFloat3(&origin), O), XMVector3Dot(N, D))));
			// plane intersection
			if (dist <= 0)
				return false;

			XMVECTOR C = XMVectorAdd(O, XMVectorMultiply(D, XMVectorReplicate(dist)));
		...
		bool Ray::intersects(const Sphere& b, float& dist, XMFLOAT3& direction_out) const
		{
			bool intersects = b.intersects(*this, dist, direction_out);
			return intersects;
		}
		...
		bool Ray::intersects(const Capsule& b, float& dist, XMFLOAT3& direction_out) const
		{
			bool intersects = b.intersects(*this, dist, direction_out);
			return intersects;
		}
		...
		bool Ray::intersects(const Plane& b, float& dist, XMFLOAT3& direction_out) const
		{
			return b.intersects(*this, dist, direction_out);
		}
		...
		void Frustum::Create(const XMMATRIX& viewProjection)
		{
			...
			// Far plane:
			XMStoreFloat4(&planes[1], XMPlaneNormalize(XMVectorSubtract(mat.r[3], mat.r[2])));

			// Left plane:
			XMStoreFloat4(&planes[2], XMPlaneNormalize(XMVectorAdd(mat.r[3], mat.r[0])));

			// Right plane:
			XMStoreFloat4(&planes[3], XMPlaneNormalize(XMVectorSubtract(mat.r[3], mat.r[0])));

			// Top plane:
			XMStoreFloat4(&planes[4], XMPlaneNormalize(XMVectorSubtract(mat.r[3], mat.r[1])));

			// Bottom plane:
			XMStoreFloat4(&planes[5], XMPlaneNormalize(XMVectorAdd(mat.r[3], mat.r[1])));
		}
		...
		{
			return Math::Collision2D(pos, siz, b.pos, b.siz);
		}
